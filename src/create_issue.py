...
from utils import (
    ...
    truncate_text, generate_issue_hash, format_duration, get_relative_path,
    format_stack_trace, set_github_output, get_branch_name, format_timestamp
)
from error_handling import (
    setup_error_handling, ActionError, ErrorCodes, ErrorSeverity,
    ConfigValidator, GitHubAPIErrorHandler, error_handler
)
try:
    from ai_analysis import analyze_failures_with_ai, AIAnalysisFormatter
...
class IssueManager:
    ...
    def create_or_update_issue(self, summary: Dict[str, Any], title: str, 
                              labels: List[str], assignees: List[str], 
                              deduplicate: bool = True, ai_analysis=None) -> Tuple[int, str, bool]:
        """
        ...
        if deduplicate:
            ...
            if existing_issue:
                print(f"Found existing issue #{existing_issue['number']}: {existing_issue['title']}")
                # Update the existing issue with new information
                updated_issue = self.github_client.update_issue(
...
def main():
    ...
    parser.add_argument('--deduplicate', default='true', help='Whether to check for existing issues')
    parser.add_argument('--ai-analysis', default='false', help='Enable AI analysis (future feature)')
